...
const countFlexLines = (container) => {
  const kids = Array.from(container.children)
  const tops = new Set(kids.map(k => k.offsetTop))

  // console.log("list top :", tops)

  return tops   // trả về set thay vì size
}

const logLineCounts = () => {
  bigSet = new Set()   // reset mỗi lần chạy

  for (let i = 0; i < paraRefs.value.length; i++) {
    const el = paraRefs.value[i]

    const tops = countFlexLines(el)   // nhận lại set
    for (let t of tops) bigSet.add(t)

    console.log(`Paragraph ${i} lines:`, tops.size)
  }
  console.log("list top :", bigSet)
  console.log('Total lines (all paras):', bigSet.size)
}
